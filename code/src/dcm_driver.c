	...
	if (motor.state == MOTION_RUNNING)
	{
		if (!(motor.fault.flag & FAULT_MASK_PHASE_A_OPEN))
		{
			if (!motor.fault.openDetectCheck)
			{
				...
				{
					setFaultDetect(FAULT_OPEN, SET, CLEAR);
				}
				else
				{
...
void app_motor_task(void)
{
	...
	case MOTION_DEC:
	{
		next_state = motor_state_DCC();
		break;
	}
	case MOTION_PAUSE:
	{
		next_state = motor_state_PAUSE();
		break;
	}
	case MOTION_STALL:
	{
		next_state = motor_state_STALLED();
		break;
	}
	case MOTION_FAULT:
	{
